            ...
            <form method="POST" action="{{ route('cameras.update', $camera) }}">
                ...
                <div class="grid grid-cols-1 gap-6">
                    <div>
                        ...
                        <input type="text" id="name" name="name" value="{{ old('name', $camera->name) }}" required class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                    </div>
                    ...
                    <div>
                        ...
                        <select id="type" name="type" required class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md">
                            <option value="usb" {{ $camera->type === 'usb' ? 'selected' : '' }}>USB Camera</option>
                            <option value="ip" {{ $camera->type === 'ip' ? 'selected' : '' }}>IP Camera (RTSP)</option>
                        </select>
                    </div>

                    <div id="usb-field" class="{{ $camera->type !== 'usb' ? 'hidden' : '' }}">
                        <label for="device_id" class="block text-sm font-medium text-gray-700">USB Device ID</label>
                        <input type="text" id="device_id" name="device_id" value="{{ old('device_id', $camera->device_id) }}" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                        <p class="mt-2 text-sm text-gray-500">Typically 0, 1, 2, etc. depending on connected devices</p>
                    </div>

                    <div id="ip-field" class="{{ $camera->type !== 'ip' ? 'hidden' : '' }}">
                        <label for="rtsp_url" class="block text-sm font-medium text-gray-700">RTSP URL</label>
                        <input type="text" id="rtsp_url" name="rtsp_url" value="{{ old('rtsp_url', $camera->rtsp_url) }}" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" placeholder="rtsp://username:password@ip:port/path">
                    </div>
                    ...
                    <div>
                        ...
                        <select id="is_active" name="is_active" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md">
                            <option value="1" {{ $camera->is_active ? 'selected' : '' }}>Active</option>
                            <option value="0" {{ !$camera->is_active ? 'selected' : '' }}>Inactive</option>
                        </select>
                    </div>
                </div>
